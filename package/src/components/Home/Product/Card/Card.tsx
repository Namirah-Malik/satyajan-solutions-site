...
// ── Bestseller — Amazon order data (Top 10 by Total Order Items) ───────────────
// Ordered by rank: index 0 = #1 bestseller
const BESTSELLER_PATTERNS = [
  'i lithium 1500',                  // rank 1 — 17 orders
  'heavy duty 1550 advanced',        // rank 2 — 13 orders
  'super power ups 900',             // rank 3 — combo 800va+150ah
  'super power ups 1100',            // rank 4 — combo 950va+150ah
  'super power 1100.*lithium',       // rank 5 — combo with lithium
  'luxe wifi.*1400',                 // rank 6 — wifi inverter
  'msmf 7.2',                        // rank 7 — VRLA battery
  'super power 1100 advanced digital', // rank 8
];

// Also keep slug-based set for DB products that have slugs
...
// ── 10% OFF products — from Excel "10 off" sheet ──────────────────────────────
const OFFER_PATTERNS = [
  '25.6v lifepo4',
  '25.6v.*lithium',
  'luxe wifi.*lithium.*combo',
  'luxe.*lifepo4.*combo',
  'msmf 7.2',
  'heavy duty 2350',
  'smart hybrid 875',
  ...
  'heavy duty 1550 sw',
  'heavy duty 1750 sw',
  'smart hybrid new 875',
  'luxe.*1400.*150ah',
  'luxe new 1400.*lifepo4',
  'luxe new.*lifepo4',
];
...
const PropertyCard: React.FC<{ item: PropertyHomes }> = ({ item }) => {
  ...
  return (
    <div className="w-full h-full group">
      <div className="relative rounded-2xl bg-white overflow-hidden flex flex-col h-full
        ...
        <div className={`absolute top-0 left-0 right-0 h-0.5 z-10 ${
          isOffer ? 'bg-gradient-to-r from-red-400 via-orange-400 to-yellow-400' :
          isBestSeller ? 'bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-400' :
            ...
            <div className="absolute top-2.5 left-2.5 z-20 flex flex-col gap-1">
              {isOffer && (
                <span className="inline-flex items-center gap-1 bg-red-500 text-white
                                 text-[9px] font-black px-2 py-0.5 rounded-full shadow-md tracking-wide">
                  🔥 10% OFF
                </span>
              )}
              {isBestSeller && (
                <span className="inline-flex items-center gap-1 bg-amber-500 text-white
                                 text-[9px] font-black px-2 py-0.5 rounded-full shadow-md tracking-wide">
                  🏆 BESTSELLER
                </span>
              )}
        ...
        <div className="flex flex-col flex-1 px-3 pb-3 pt-2.5 sm:px-4 sm:pb-4 sm:pt-3 gap-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="inline-flex items-center gap-1 text-[9px] sm:text-[10px] font-semibold
                             text-emerald-600 bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 rounded-full">
          ...
          <Link href={slug ? `/products/${slug}` : '#'} className="block">
            <h3 className="text-[12px] sm:text-[13px] font-bold text-gray-800 leading-snug line-clamp-2
          ...
          {capacity && (
            <div className="inline-flex items-center gap-1.5 w-fit
                            ...
                            text-[10px] sm:text-[11px] font-bold px-2 py-1 rounded-lg">
          ...
          <button
            ...
            disabled={adding}
            className={`w-full py-2 sm:py-2.5 rounded-xl font-bold text-[11px] sm:text-[13px]
          ...
          <div className="grid grid-cols-2 gap-1.5">
            <a
              ...
              className="flex items-center justify-center gap-1 sm:gap-1.5
                         border border-gray-200 hover:border-gray-400 bg-white hover:bg-gray-50
                         text-gray-700 font-bold rounded-xl py-1.5 sm:py-2
                         text-[10px] sm:text-[11px] transition-all duration-200 active:scale-95"
            >
              <Icon icon="solar:phone-bold" width={12} className="text-gray-600" />
              Call Us
            </a>
            <a
              href={waPhone} target="_blank" rel="noopener noreferrer"
              className="flex items-center justify-center gap-1 sm:gap-1.5
                         bg-[#25D366] hover:bg-[#20c05c] text-white font-bold
                         rounded-xl py-1.5 sm:py-2 text-[10px] sm:text-[11px]
...
// ── Export helpers for ProductsClient to use ──────────────────────────────────
