...
// ── Stable rating from slug (seeded, not random) ──────────────────────────────
// Returns a rating between 4.5 and 5.0, consistent per product
function getProductRating(slug: string): number {
  ...
  // Map to 4.5–5.0 in steps of 0.1 → 6 possible values
  const steps = [4.5, 4.6, 4.7, 4.8, 4.9, 5.0];
  return steps[abs % steps.length];
}

// ── Star rating review count (seeded) ────────────────────────────────────────
...
function StarRating({ rating, reviewCount, compact = false }: { rating: number; reviewCount: number; compact?: boolean }) {
    ...
    <div className={compact ? 'flex flex-col gap-0.5' : 'flex flex-col gap-1'}>
      <div className="flex items-center gap-1">
        ...
        {Array.from({ length: fullStars }).map((_, i) => (
          <Icon key={`f${i}`} icon="ph:star-fill" className="text-yellow-400" width={compact ? 11 : 13} />
        ))}
        {/* Half star */}
        {hasHalf && <Icon icon="ph:star-half-fill" className="text-yellow-400" width={compact ? 11 : 13} />}
        {/* Empty stars */}
        {Array.from({ length: emptyStars }).map((_, i) => (
          <Icon key={`e${i}`} icon="ph:star" className="text-yellow-300" width={compact ? 11 : 13} />
        ))}
        <span className={`font-bold text-gray-700 ${compact ? 'text-[10px]' : 'text-xs'}`}>{rating.toFixed(1)}</span>
        <span className={`text-gray-400 ${compact ? 'text-[9px]' : 'text-[10px]'}`}>({reviewCount})</span>
      </div>
      <p className={`text-gray-400 leading-tight ${compact ? 'text-[8px]' : 'text-[9px]'}`}>
        Based on customer reviews from Google &amp; IndiaMART
      </p>
    </div>
  );
}
...
const PropertyCard: React.FC<{ item: PropertyHomes }> = ({ item }) => {
  ...
  const rating      = getProductRating(slug || name);
  const reviewCount = getReviewCount(slug || name);
        ...
        <Link href={slug ? `/products/${slug}` : '#'} className="block flex-shrink-0 relative">
          <div className="relative w-full overflow-hidden bg-gradient-to-b from-gray-50 to-white" style={{ aspectRatio: '1/1' }}>
            ...
            {showImage ? (
              ...
              <img src={mainImage!} alt={name} loading="lazy" onError={() => setImgError(true)}
